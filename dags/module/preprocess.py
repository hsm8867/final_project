...
def process_data(df):
    ...
    df['dateAfter7Days'] = df['openDt'].apply(lambda x: (datetime.datetime.strptime(x, '%Y-%m-%d') + timedelta(days=7)).strftime('%Y-%m-%d'))
    ...
    for i in range(len(df)):
        ...
        movieCd = row['movieCd']
        openDt = row['openDt']
        df_search = df[(df['date'] <= df['dateAfter7Days']) & (df['movieCd'] == movieCd)]
