...
async def load_data(engine: AsyncSession) -> pd.DataFrame:
    # This query fetches the data while excluding rows with NaN values in relevant columns
...
def train_catboost_model_fn(**context: dict) -> None:
    ...
    ti = context["ti"]
    hook: PostgresHook = PostgresHook(postgres_conn_id="POSTGRES_DEFAULT")
    db_uri = hook.get_uri()
    ...
    # Optuna 설정
    postgresHook: PostgresHook = PostgresHook(postgres_conn_id="HYPERPARAMETER_STORE")
